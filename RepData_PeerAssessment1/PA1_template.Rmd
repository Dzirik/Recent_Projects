...
There is obvious that the maximum value is only one and it is the value `r maxValue` in interval number `r inIntervalNumber` labeled as `r inIntervalLabeled`.
...
1. The problem of the rawDataset are missing values.
    ...
    ```  
There are `r nOfMissingSteps` missing values in column Steps and fortunately `r nOfMissingDate` and `r nOfMissingInterval` in columns date and interval.
...
3. Histogram of updated data.
    ...
    ```
4. The mean value of updated dataset is `r meanStepsPerDayWithouNAs` and the median is `r 
medianStepsPerDayWithoutNAs`. These values differs from values from dataset with NAs. This is due to the methodology of replacing the NAs. In our approach the 5 min interval behaviour was preserved, but the mean and median value not.
